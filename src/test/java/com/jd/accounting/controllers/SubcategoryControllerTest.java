...
//        account2.setName("CCP");
//        account2.setInitial(0);
...
//        Mockito.when(categoryService.create(Mockito.any(), Mockito.any())).thenReturn(category);
        ...
        Mockito.when(subcategoryService.create(Mockito.any(), Mockito.any())).thenReturn(subcategory);

        // Test nominal
        Mockito.when(userService.getCurrentUser()).thenReturn(userUser);
        mockMvc.perform(post("/categories/1/subcategories")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(subcategory))
